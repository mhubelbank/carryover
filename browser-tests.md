...
- [ ] Teachers tab → a filming-capable teacher (Alfredo/Lefkie) shows a role checkbox picker (no inline role editor); toggling writes filmingRoleIds; "Manage catalog" jumps to Activities.
- [ ] Generate · filming day → Alfredo: role dropdown lists Anchor + Reporter (his filmingRoleIds); picking a role surfaces that role's field-components; note generates correctly.
...
- [ ] Generate → a Bengali student under Joanne still gets the Bengali capture (now via `language includes "Bengali"`); a Spanish student under Nina gets the Spanish append; Nina journal renders the method word.
