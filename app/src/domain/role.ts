...
// A teacher's filming-day roles, resolved from the shared catalog. Dangling ids
...
export function resolveRoles(teacher: Teacher, catalog: Role[]): Role[] {
  ...
  return teacher.filmingRoleIds.map((id) => byId.get(id)).filter((r): r is Role => r != null);
}
...
export function roleRefCounts(teachers: Teacher[]): Map<string, number> {
  ...
  for (const t of teachers) {
    for (const id of t.filmingRoleIds) counts.set(id, (counts.get(id) ?? 0) + 1);
  }
  return counts;
}
