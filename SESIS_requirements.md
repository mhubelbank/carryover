...
- **`data/teachers.json`** — id (stable, name-independent), name (display label), color (one of 12 palette options), supported modes (regular, filming-day), activities[], roles[] (filming-day only), per-student field declarations (e.g. Dana's `needsBengali`), optional prompt overrides
...
- **Per-student fields**: declared here, rendered on student detail (e.g. Dana declares "Bengali support: bool")
