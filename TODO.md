...
- **Per-teacher quirk handlers — DONE (verified live + harness).** All run
  ...
  not bespoke code:
  - ✅ **Dana Bengali** — `bengali` capture (`bengaliUsed`/`bengaliDetails`
    fields → `promptInjection` into `{{additionalContext}}`). Confirmed live.
  - ✅ **Robin Spanish** — `spanish` capture `postProcess.appendToFinalNote`.
    Confirmed live.
  - ✅ **Robin journal** — `journal` capture `activityDescriptionTemplate` with
...
- **Sync the activity-catalog migration to the `data` branch.** The code now reads
  ...
  Robin's journal `sessionCapture` folded onto the `a_journal` catalog entry). Sync
