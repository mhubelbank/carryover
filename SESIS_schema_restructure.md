...
Alfredo, José, and Lefkie have no `sessionCaptures` (empty array or omit).

## Three capture types to support

1. **Form fields + prompt injection** (Joanne's Bengali): renders extra UI in the Generate form when the condition is met; appends to `additionalContext` when the session value is set.
2. **Post-process append** (Nina's Spanish): no UI, no LLM call — deterministic string append to the final note when the condition is met. Bypasses the pipeline entirely.
3. **Activity description rewrite** (Nina's journal): interpolates the student attribute into the activity description string *before* it goes into the prompt. No extra form fields.
...
For each teacher's preset roster in the original TSX files, port student records into `data/students.csv` with the attributes above. Where an attribute didn't exist on that teacher's preset (e.g., Lefkie's students never had `aacDevice`), leave the column empty.
